"""
Whole code taken from Deeplift repo.
Placed here to avoid dependencies
...
from deeplift.dinuc_shuffle import traverse_edges, shuffle_edges, prepare_edges
...
# # shap.explainers._deep.deep_tf.op_handlers["Erf"] = shap.explainers._deep.deep_tf.passthrough
...
shuffle_several_times = lambda s,times: np.array([onehot_dinuc_shuffle(s) for i in range(times)])


def compute_shap_score(model,seq_to_explain):
    ### seq_to_explain is [1,input_width,4]
    # get background
    background = shuffle_several_times(seq_to_explain[0],100)
    # create explainer with model, backrgound
    dinuc_shuff_explainer = shap.DeepExplainer(model,background)
    # get shap values for the example
    raw_shap_explanations = dinuc_shuff_explainer.shap_values(seq_to_explain)[0] # this extra 0 must be added since this shap values functions returns a list
    # get per-base pair importance
    dinuc_shuff_explanations = np.sum(raw_shap_explanations,axis=-1)[:,:,None]*seq_to_explain 
    # return
