### score all - not just regulatory

import os
mutations_root="/illumina/scratch/deep_learning/akumar22/TCGA/mutation_prioritization/somatic_filtered/"
exptroot="/illumina/scratch/deep_learning/akumar22/TCGA/mutation_prioritization/somatic_filtered/"
...
for c_idx,cancer_name in enumerate(all_cancers):
    mutations_file = f"{mutations_root}/{cancer_name}_filtered_somatic.csv"
    for fold_num in range(5):
        ...
        outfile =  f"{exptroot}/{cancer_name}/fold_{fold_num}_somatic_mutation_centered.pkl"
        outdir =  f"{exptroot}/{cancer_name}/logs/"
        
        if not os.path.exists(outdir):
            os.makedirs(outdir)

        jobname = f"{cancer_name}_fold{fold_num}_somatic_mutation_centered"
