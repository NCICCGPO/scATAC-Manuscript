...
def load_bpnet_chkpt(path_to_weights_h5: str, verbose=False):
    assert os.path.exists(path_to_weights_h5), 'invalid model file!'
    model = get_bpnet_model(1364, 8)
    model.load_weights(path_to_weights_h5)
    if verbose:
        print(model.summary())
    return model

def get_bpnet_folds(h5_file_format: str = './tmp/TCGA/models_250_1364_minibatch_prejitter/BRCA/fold_{}/model.h5',):
    model_state_files = [h5_file_format.format(i) for i in range(5)]
    model_folds = {}
    for i, f in enumerate(model_state_files):
        model_folds[i] = load_bpnet_chkpt(f)
    return model_folds

def get_bpnet_output(X_in, model):
    return model(X_in).numpy()
